...
const collections=['stock','machines','bom','suppliers','movements','purchases','ops','maintenance','warranties'];
...
export function AppProvider({children}){
 ...
 const setStock=makeSetter('stock',setStockRaw);const setMachines=makeSetter('machines',setMachinesRaw);const setBom=makeSetter('bom',setBomRaw);const setSuppliers=makeSetter('suppliers',setSuppliersRaw);const setMovements=makeSetter('movements',setMovementsRaw);const setPurchases=makeSetter('purchases',setPurchasesRaw);const setOps=makeSetter('ops',setOpsRaw);const setMaintenance=makeSetter('maintenance',setMaintenanceRaw);const setWarranties=makeSetter('warranties',setWarrantiesRaw);
 settersRef.current={stock:setStockRaw,machines:setMachinesRaw,bom:setBomRaw,suppliers:setSuppliersRaw,movements:setMovementsRaw,purchases:setPurchasesRaw,ops:setOpsRaw,maintenance:setMaintenanceRaw,warranties:setWarrantiesRaw};
 statesRef.current={stock:stockRaw,machines:machinesRaw,bom:bomRaw,suppliers:suppliersRaw,movements:movementsRaw,purchases:purchasesRaw,ops:opsRaw,maintenance:maintenanceRaw,warranties:warrantiesRaw};
 ...
 const stock=stockRaw,machines=machinesRaw,bom=bomRaw,suppliers=suppliersRaw,movements=movementsRaw,purchases=purchasesRaw,ops=opsRaw,maintenance=maintenanceRaw,warranties=warrantiesRaw;
 ...
 const backup=()=>downloadJson('backup-almoxarifado-gift.json',{stock,machines,bom,suppliers,movements,purchases,ops,maintenance,warranties,settings,createdAt:new Date().toISOString()});
 ...
 const resetDemo=()=>{setStock([]);setMachines([]);setBom([]);setSuppliers([]);setMovements([]);setPurchases([]);setOps([]);setMaintenance([]);setWarranties([]);notify('Sistema zerado')};
